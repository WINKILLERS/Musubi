...
/*////////////////////////////////////////////////////////////////////////////////*\
Simple implementation of a Linear Congruential Pseudo-Random Number Generator
(PRNG) https://en.wikipedia.org/wiki/Linear_congruential_generator
...
// This ensures that RNG() is evaluated at compile time! (Keyword: Template
// Metaprogramming)
template <uint32_t Const> struct MakeConst {
  enum { Value = Const };
};
...
#define LCG_RANDOM_IN_RANGE(min, max)                                          \
  (min + (LCG_RANDOM() & m) % (max - min + 1))
// This acts as a bool. Could have use a range of 0 and 1, but this might have
